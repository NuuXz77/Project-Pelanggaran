...
new class extends Component {
    ...
    public function updateBk()
    {
        ...
        $this->toast('success', 'Berhasil!', 'Data akun BK berhasil diperbarui');
...
<div>
    <x-modal wire:model="modalEdit" title="Edit Akun BK" subtitle="Perbarui informasi akun BK" separator persistent>
        <x-form wire:submit="updateBk">
            <x-input label="Nama" wire:model="name" icon="o-user" />
            <x-input label="Email" wire:model="email" icon="o-envelope" />
            <x-select label="Status" wire:model="status" :options="[['id'=>'aktif','name'=>'Aktif'],['id'=>'nonaktif','name'=>'Tidak Aktif']]" option-label="name" option-value="id" />

            <x-slot:actions>
                <x-button label="Batal" @click="$wire.modalEdit = false" />
                <x-button label="Simpan Perubahan" type="submit" class="btn-primary" spinner="updateBk" />
            </x-slot:actions>
        </x-form>
    </x-modal>
</div>
