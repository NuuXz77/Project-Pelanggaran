<div class="bg-base-100 p-6 rounded-lg shadow-sm breadcrumbs text-sm">
    <div class="breadcrumbs text-sm">
        <ul>
            ...
            <li><span><x-icon name="o-user-group" /> Akun GURU</span></li>
        </ul>
    </div>

    <x-header title="Data Akun GURU" subtitle="Daftar akun pengguna dengan role GURU" separator progress-indicator>
        <x-slot:middle class="!justify-end">
            <x-input wire:model.live.debounce.500ms="search" icon="o-magnifying-glass"
                placeholder="Cari nama atau email..." />
        </x-slot:middle>
        <x-slot:actions>
            <livewire:akun-guru.add-guru />
        </x-slot:actions>
    </x-header>
    ...
    <x-table :headers="$headers" :rows="$users" :sort-by="$sortBy" per-page="perPage" :per-page-values="[5, 10, 15]" with-pagination
        ...
        <x-slot:empty>
            <x-icon name="o-archive-box" label="Tidak ada data akun guru" />
        </x-slot:empty>
    </x-table>

    <livewire:akun-guru.edit-guru />
    <livewire:akun-guru.delete-guru />
    <livewire:akun-guru.reset-guru />
</div>
