...
new class extends Component {
    ...
    public $akun_id;
    public $nama;
    public string $defaultPassword = 'pkssmkn1ciamis'; // ← bisa kamu ubah kapan saja
    ...
    public function openModal($id)
    {
        ...
        if ($akun && $akun->role === 'pks') {
            $this->akun_id = $akun->ID_Akun;
            $this->nama = $akun->name;
            $this->resetModal = true;
        } else {
            $this->toast(type: 'error', title: 'Error', description: 'Akun PKS tidak ditemukan.', icon: 'o-x-circle', css: 'alert-error');
        }
    }
