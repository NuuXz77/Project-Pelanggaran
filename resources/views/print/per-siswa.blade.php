...
<head>
    ...
    <title>Data Siswa dan Pelanggaran</title>
    <style>
        @page {
            size: A4 landscape;
            margin: 15px;
...
<body>
    <div class="print-container">
        ...
        <!-- Judul Laporan -->
        <div class="report-title">LAPORAN DATA SISWA DAN REKAPITULASI PELANGGARAN</div>

        <!-- Filter Info -->
        @if($filter['search'] || $filter['nama_siswa'] || $filter['kelas'] || $filter['tanggal_awal'] || $filter['tanggal_akhir'])
            <div class="filter-info">
                ...
                @if($filter['nama_siswa'])
                    <div class="filter-item"><strong>Nama Siswa:</strong> {{ $filter['nama_siswa'] }}</div>
                @endif
                @if($filter['kelas'])
        ...
        <table>
            <thead>
                ...
                <tr>
                    @foreach($jenisPelanggaran as $jenis)
                        <th class="pelanggaran-col" title="{{ $jenis }}">
                            {{ $aliasMapping[$jenis] }}
                        </th>
                    @endforeach
                </tr>
            ...
            <tfoot>
                <tr style="background-color: #e9ecef; font-weight: bold;">
                    ...
                    @foreach($jenisPelanggaran as $jenis)
                        <td>
                            {{ $siswaData->sum(function($data) use ($jenis) {
                                return $data['pelanggaran'][$jenis] === '-' ? 0 : $data['pelanggaran'][$jenis];
                            }) }}
                        </td>
                    @endforeach
                    <td>
                        <strong>{{ $siswaData->sum('total') }}</strong>
                    </td>
                </tr>
            </tfoot>
        </table>
